    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                fogCol *= _MieIntens * smoothstep(1 - _MieSize, 1, saturate(dot(float2(cosa, sina), i.texcoord.xz))) *
                    _MieTint.rgb * unity_ColorSpaceDouble.rgb + 1;

                //Apply Fog
                c = lerp(c, fogCol, saturate(
                             (1 - smoothstep(_FogStart * 2, _FogEnd * 2,
                                             (_FogBottom == 0 ? abs(i.texcoord.y) : i.texcoord.y))) * _FogIntens));
